// Decompiled with JetBrains decompiler
// Type: QRCerts.Api.Services.IOtecService
// Assembly: QRCerts.Api, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: 727CB6D4-E29D-47B1-9E4A-93BB4836AB5B
// Assembly location: C:\Users\allco\Downloads\QRCerts_full\src\QRCerts.Api\bin\Release\net8.0\publish\QRCerts.Api.dll

using QRCerts.Api.Models;
using System;
using System.Collections.Generic;

#nullable enable
namespace QRCerts.Api.Services
{
  public interface IOtecService
  {
    List<Otec> GetAll();

    List<Otec> GetActive();

    Otec GetById(Guid id);

    Otec GetBySlug(string slug);

    bool SlugExists(string slug, Guid? excludeId = null);

    int GetUserCount(Guid otecId);

    Guid Insert(Otec otec);

    void Update(Otec otec);

    void Delete(Otec otec);
  }
}
